...
class TestSlider(BaseTest):
    ...
    def test_slider(self, page, base_url, slide_index):
        """
        ...
        try:
            with allure.step(f"Перейти на страницу со слайдером ({base_url})"):
                ...
                self.slider_page.go_to(base_url)
                end_time = time.time()
                logger.info(
                    f"Страница со слайдером открыта за "
                    f"{end_time - start_time:.2f} секунд."
                )

            with allure.step(f"Кликнуть на слайд с индексом {slide_index}"):
                start_time = time.time()
                self.slider_page.click_slide(slide_index)
                end_time = time.time()
                logger.info(
                    f"Кликнули на слайд с индексом {slide_index} "
                    f"за {end_time - start_time:.2f} секунд."
                )

            with allure.step(
                f"Проверить, что слайд с индексом " f"{slide_index} активен"
            ):
                start_time = time.time()
                is_slide_active = self.slider_page.is_slide_active(slide_index)
                end_time = time.time()

                assert is_slide_active, (
                    f"Слайд с индексом " f"{slide_index} не активен."
                )
                if is_slide_active:
                    logger.info(
                        f"Слайд с индексом {slide_index} активен "
                        f"за {end_time - start_time:.2f} секунд."
                    )
                else:
                    logger.error(
                        f"Слайд с индексом {slide_index} не "
